...
class SDnetConditioner(nn.Module):
    ...
    def forward(self, input):
        ...
        weights = (e1, e2, e3, e4, bn, d4, d3, d2, d1, conv_w)
...
class SDnetSegmentor(nn.Module):
    ...
    def forward(self, inpt, weights=None):
        e_w1, e_w2, e_w3, e_w4, bn_w, d_w4, d_w3, d_w2, d_w1, conv_w = weights
        ...
        if conv_w is not None:
            channels = conv_w.size()[0]
            conv_w = conv_w.view(1, channels, 1, 1)
            logit = F.conv2d(d1, conv_w)
