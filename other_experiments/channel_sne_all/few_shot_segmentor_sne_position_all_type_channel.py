...
class SDnetConditioner(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.channel_conv_e3 = nn.Linear(params['num_filters'], 64, bias=True)

        self.encode4 = sm.SDnetEncoderBlock(params)
        self.channel_conv_e4 = nn.Linear(params['num_filters'], 64, bias=True)

        self.bottleneck = sm.GenericBlock(params)
        self.channel_conv_bn = nn.Linear(params['num_filters'], 64, bias=True)
        params['num_channels'] = 16
    ...
    def forward(self, input):
        ...
        num_batch, ch, _, _ = out3.size()
        e_c3 = self.sigmoid(self.channel_conv_e3(out3.view(num_batch, ch, -1).mean(dim=2)))
        num_batch, ch = e_c3.size()
        e_c3 = e_c3.view(num_batch, ch, 1, 1)

        e4, out4, ind4 = self.encode4(e3)
        num_batch, ch, _, _ = out4.size()
        e_c4 = self.sigmoid(self.channel_conv_e4(out4.view(num_batch, ch, -1).mean(dim=2)))
        num_batch, ch = e_c4.size()
        e_c4 = e_c4.view(num_batch, ch, 1, 1)

        bn = self.bottleneck(e4)
        num_batch, ch, _, _ = bn.size()
        bn_c = self.sigmoid(self.channel_conv_bn(bn.view(num_batch, ch, -1).mean(dim=2)))
        num_batch, ch = bn_c.size()
        bn_c = bn_c.view(num_batch, ch, 1, 1)

        d4 = self.decode4(bn, None, ind4)
        num_batch, ch, _, _ = d4.size()
        d_c4 = self.sigmoid(self.channel_conv_d4(d4.view(num_batch, ch, -1).mean(dim=2)))
        num_batch, ch = d_c4.size()
        d_c4 = d_c4.view(num_batch, ch, 1, 1)
        ...
        space_weights = (e_c1, e_c2, e_c3, e_c4, bn_c, d_c4, d_c3, d_c2, d_c1, None)
        channel_weights = (e_c1, e_c2, e_c3, e_c4)
...
class SDnetSegmentor(nn.Module):
    ...
    def forward(self, inpt, weights=None):
        ...
        e_c1, e_c2, e_c3, e_c4 = channel_weights
